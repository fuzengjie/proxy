...
class ProxyHandler(tornado.web.RequestHandler):
    ...
    def authentication(func):
        def inner(self,*args,**kwargs):
            ...
            if not auth_header:
                self.set_status(407)
                self.set_header("Proxy-Authenticate","Basic realm=Proxy Auth")
                self.write("Proxy Authenticate Faild")
                self.finish()
                return
            else:
                auth_mode, auth_base64 = auth_header.split(' ', 1)
                auth_username, auth_password = auth_base64.decode('base64').split(':', 1)
                if auth_username == 'fuzj' and auth_password == '123':
                    fetch_main = func(self,*args,**kwargs)
                    return fetch_main
                else:
                    self.set_status(407)
                    self.set_header("Proxy-Authenticate","Basic realm=Proxy Auth")
