...
def getUser( datas ):
	...
	auth_s = datas.find("Authorization: Basic ")
	if auth_s > 0:
		auth_e = datas.find("\r\n", auth_s)
		auth_str = datas[auth_s+21:auth_e].strip()
		auth_str = base64.b64decode( auth_str )
		add_list = auth_str.split(":")
		if len(add_list) == 2:
			username = add_list[0]
			password = add_list[1]
...
def checkAuthorization( username, password ):
	if not username or not password :
		return False
	if username == 'fuzj' and password == '123':
		return True
	else:
		return False
	nowdatetime = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S');
	db = MySQLdb.connect(host='localhost',user='root',passwd='',db='proxy',port=3306)
	cursor = db.cursor()
	cursor.execute('set names utf8')
	cursor.execute("select count(1) from proxy_auth_user where username='%s' and password='%s' and begintime<='%s' and endtime>='%s' and status=1 " % \
	( username, password, nowdatetime, nowdatetime ) );
...
def connect_pro(params, param, epoll_fd, datas, fd, cur_time, host, port):
	try:
		...
		try:
			...
			print "%s:%s connect fail!\n" % (host,port)
			param["writedata"] = "%s:%s connect fail!\n" % (host,port);
			param["writelen"] = 0
			epoll_fd.modify(fd, select.EPOLLOUT | select.EPOLLERR | select.EPOLLHUP)
...
def process_datas(process_status, params, param, epoll_fd, datas, read_len, fd, cur_time):
	if process_status == "close":
		...
		if need_connect or not "nextfd" in param:
			username,password = getUser(datas);
			print 'nnnnnnnnnnnnnnnnnnnnnnnnn';
			print username,password
			if checkAuthorization( username,password ) == False :
				print 'illegal user!';
				print fd;
				#param["writedata"] = 'HTTP/1.1 407 Unauthorized\r\nProxy-Authenticate: Basic realm=\"USER LOGIN\"\r\n\r\n';
		   		param["writedata"] = "HTTP/1.1 407 Connection Established\r\nProxy-Authenticate: Basic realm=\"USER LOGIN\"\r\n\r\n"
				#param["writedata"] = "illegal user!\n";
		   		#param["writedata"] = "HTTP/1.1 200 Connection Established\r\nConnection: close\r\n\r\n"
				param["writelen"] = 0
				param["sendandclose"] = True;
				epoll_fd.modify(fd, select.EPOLLOUT | select.EPOLLERR | select.EPOLLHUP)
				return None
